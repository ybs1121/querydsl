...
//        BooleanBuilder builder = new BooleanBuilder(member.username.eq(usernameCond)); // 초기값 세팅
...
//        Integer ageParam = null;
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond)) // Where에 null이 들어가면 무시
                .fetch();
    }
    ...
    public void bulkUpdate() {
        ...
        List<Member> result = queryFactory.selectFrom(member).fetch(); // DB VS 영속성 컨텍스트 , 영속성 컨텍스트 우선권을 가진다.
